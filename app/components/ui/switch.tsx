...
function Switch({ className, label, ...props }: Props) {
  return (
    <SwitchPrimitive.Root
      data-slot="switch"
      className={cn(
        "peer data-[state=unchecked]:bg-input focus-visible:border-ring focus-visible:ring-ring/50 dark:data-[state=unchecked]:bg-input/80 relative inline-flex h-[2rem] w-28 shrink-0 cursor-pointer items-center rounded-full border border-transparent shadow-xs transition-all outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:bg-green-600",
        className
      )}
      {...props}
    >
      ...
      />
      <p className="peer absolute left-1/2 -translate-x-[calc(65%)] text-base text-white peer-data-[state=unchecked]:hidden">
        {label}
      </p>
      <p
        data-slot="switch-label"
        className="text-foreground absolute right-1/2 -translate-x-[calc(-75%)] text-base peer-data-[state=checked]:hidden"
      >
        {label}
      </p>
    </SwitchPrimitive.Root>
